function [entropy]=pl_gaudin_entropy(h,mu,Temperature)
...
mu1=mu+h/2;
mu2=mu+1/4;
mu1=mu1*(sign(mu1)+1)/2;
mu2=mu2*(sign(mu2)+1)/2;

kc1=max(2*sqrt(mu1),sqrt(mu1+10*T));
...
kc2=max(2*sqrt(mu2),sqrt(mu2+10*T));
...
tv_add=linspace(-bound*0.7,bound*0.7,NUM_int);
...
entropy=-1/pi/2*sum(v_k1_nt.*(v_g_ep1.*v_ep1_dT+v_h1_ep1))...

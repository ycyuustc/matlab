...
vx1 = fn_onepoint(vx,v_epsilon,g,1);
disp(fn_Fj(vx1,v_epsilon,g));
...
function [m_close_index,v_ind] = fn_judge(vx,v_epsilon,dis)

[mx1,mx2] = meshgrid(vx,vx);
dm = mx1-mx2;
dm(logical(triu(ones(length(vx))))) = 1;
dm = abs(dm);
[vind2,vind1] = find(dm<dis);
m_close_index = [vind1,vind2];
v_ind = zeros(1,length(vind1));
...
function vx = fn_onepoint(vx,v_epsilon,g,index)
...
while 1
   ...
   f = sum(1./(x-v_epsilon)) - 2*sum(1./(x-vxr)) - 2/g;
   df = -sum(1./(x-v_epsilon).^2) + 2*sum(1./(x-vxr).^2);

...
v_ind_solve_2 = v_ind_solve;
Num_solve = length(v_ind_solve)/2;

m_select = zeros(Num_solve,2);
for k=1:Num_solve
   m_select(k,1) = v_ind_solve(k);
   m_select(k,2) = v_ind_solve(2*Num_solve+1-k);
end
% v_ind_solve_1 = [v_ind_solve(1),v_ind_solve(end)];
% v_ind_solve_2 = v_ind_solve;
% v_ind_solve_2(end) = [];
% v_ind_solve_2(1) = [];
...
while 1
    ...
    v_ind_solve_1 = m_select(k,:);
    
    tm = fn_m(vx,epsilon);
    vFj = fn_Fj(vx,epsilon);    
    tm = tm(v_ind_solve_1,v_ind_solve_1);
    vFj = vFj(v_ind_solve_1);  
    delta_vx = -tm\transpose(vFj);
    vx_pre = vx;
    vx(v_ind_solve_1) = vx(v_ind_solve_1) + step*transpose(delta_vx);
    
    end
    vx = (vx - vx(end:-1:1))/2;  
    tm = fn_m(vx,epsilon);
    vFj = fn_Fj(vx,epsilon);    
    tm = tm(v_ind_solve_2,v_ind_solve_2);
    vFj = vFj(v_ind_solve_2);  
    delta_vx = -tm\transpose(vFj);
    vx(v_ind_solve_2) = vx(v_ind_solve_2) + step*transpose(delta_vx);

...
M1 = ncon({hamil,hamil,hamil,hamil,hamil},...
    {[-1,-2,-11,-12],[-3,-4,-13,-14],[-5,-6,-15,-16],...
    [-7,-8,-17,-18],[-9,-10,-19,-20]});
M2 = ncon({hamil,hamil,hamil,hamil,hamil},...
    {[-2,-3,-12,-13],[-4,-5,-14,-15],[-6,-7,-16,-17],...
    [-8,-9,-18,-19],[-10,-1,-20,-11]});
...
TAB = ncon({TA,sAB,TB,sBA},{[-1,1,-3],[1,2],[2,3,-4],[3,-2]});
V = ncon({TAB,TAB,TAB,TAB,TAB},{[1,2,-1,-2],[2,3,-3,-4],[3,4,-5,-6],...
    [4,5,-7,-8],[5,1,-9,-10]});
M1 = reshape(M1,2^10,2^10);
M2 = reshape(M2,2^10,2^10);
V0 = reshape(V,[2^10,1]);
...
%     end   
    ...
    V = reshape(V,[2^10,1]);
    V0 = M2*M1*V0;
    disp(max(abs(V/norm(V)-V0/norm(V0))));

...
lieb = [3,4,1,2,0];
disp(fn_count_perc(lieb));

v_lambda = linspace(0.1,1,100);
v_percolation = zeros(1,100);
for k1 = 1:100
    
    num_count = 100;
    v_perc = zeros(1,num_count);
    lambda = v_lambda(k1);
    for k=1:num_count        
        lieb = fn_generate_lieb(lambda,N);
        v_perc(k) = fn_count_perc(lieb);
    end
    v_percolation(k1) = mean(v_perc)/N;
    disp(k1);
    
end


figure(4); hold on; plot(v_lambda,v_percolation);
% c_lieb = cell(1,M);
% storage_up = cell(1,M);
...
%         lieb_try = fn_lieb_prod(lieb_up,lieb_random);
%         lieb_try = fn_lieb_prod(lieb_try,lieb_down);
%         circle_number_try = fn_lieb_contract(lieb_try);
%         prob = 1/(1+2^(circle_number-circle_number_try));
%         
%         if rand()<prob
%             circle_number = circle_number_try;
%             c_lieb{index} = lieb_try;
%         end
%         storage_up{index+1} = ...
...
function res = fn_count_perc(lieb0)
